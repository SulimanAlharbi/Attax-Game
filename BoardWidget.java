...
class BoardWidget extends Pad  {
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        g.drawRect(0, 0, _dim, _dim);
        g.fillRect(0, 0, _dim, _dim);
        for (int i = 0; i <= _dim / SQDIM; i += 1) {
        ...
        for (char rows : ROWS) {
            ...
            vals.put(rows, cy);
            for (char cols : COLS) {
                PieceColor currentPiece = _model.get(cols, rows);
                vals.put(cols, cx);
                if (currentPiece == EMPTY) {
                    g.setColor(BLANK_COLOR);
                } else if (currentPiece == RED) {
                    g.setColor(RED_COLOR);
                } else if (currentPiece == BLUE) {
                    g.setColor(BLUE_COLOR);
                }
                g.drawOval(cx + 10, cy + 10,
                        PIECE_RADIUS * 2, PIECE_RADIUS * 2);
                g.fillOval(cx + 10, cy + 10,
                        PIECE_RADIUS * 2, PIECE_RADIUS * 2);
        ...
        if (_selectedRow != 0) {
            ...
            int sx = (Integer) vals.get(_selectedCol) + 1;
            int sy = (Integer) vals.get(_selectedRow) + 1;
            g.drawRect(sx, sy, SQDIM - 2, SQDIM - 2);
            g.fillRect(sx, sy, SQDIM - 2, SQDIM - 2);
        }
    }
    ...
    /** mapping of vals to rows and cols. */
