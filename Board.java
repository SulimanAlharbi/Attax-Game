...
class Board {
    ...
    boolean legalMove(Move move) {
        ...
        PieceColor currentPiece = get(move.col0(), move.row0());
        if (currentPiece == _whoseMove) {
            if (move.isPass()) {
                return true;
            } else {
    ...
    boolean canMove(PieceColor who) {
        if (totalOpen() == 0 || _numJumps == JUMP_LIMIT
        ...
        for (char rows : ROWS) {
            for (char cols : COLS) {
                if (get(cols, rows) == who) {
                    for (int r = -2; r <= 2; r += 1) {
                        for (int c = -2; c <= 2; c += 1) {
                            char currentRow = (char) (rows + r);
                            char currentCol = (char) (cols + c);
                            if (get(currentCol, currentRow) == EMPTY) {
                                return true;
                            }
                        }
                    }
    ...
    void makeMove(Move move) {
        ...
        for (int r = -1; r <= 1; r += 1) {
            for (int c = -1; c <= 1; c += 1) {
                char currentRow = (char) (move.row1() + r);
                char currentCol = (char) (move.col1() + c);
                if (get(currentCol, currentRow) == opponent) {
                    set(currentCol, currentRow, whoseMove());
                    _numPieces[whoseMove().ordinal()] += 1;
                    _numPieces[opponent.ordinal()] -= 1;
                    addUndo(index(currentCol, currentRow));
                }
            }
        }
        if (move.isExtend()) {
    ...
    /** Undo the last move. */
    void undo() {
        boolean doneUndo = false;
        if (_undoSquares.lastElement() == null) {
            doneUndo = false;
        } else {
            while (!doneUndo) {
                if (_undoSquares.lastElement() == null
                        || _undoPieces.lastElement() == null) {
                    _undoSquares.pop();
                    _undoPieces.pop();
                    doneUndo = true;
                } else {
                    int currentSq = _undoSquares.pop();
                    _numPieces[get(currentSq).ordinal()] -= 1;
                    PieceColor prevColor = _undoPieces.pop();
                    unrecordedSet(currentSq, prevColor);
                    _numPieces[prevColor.ordinal()] += 1;
                }
            }
            _whoseMove = _whoseMove.opposite();
            _numMoves -= 1;
            if (_numMoves == 0) {
                ...
                _undoJumps.pop();
                _numJumps = _undoJumps.get(_undoJumps.size() - 1);
            }
    ...
    boolean legalBlock(char c, char r) {
        char verticalOffset = (char) ('a' + 'g' - c);
        char horizontalOffset = (char) ('1' + '7' - r);
        if (_numMoves > 0) {
            return false;
        }
        if (get(c, r) != EMPTY) {
        ...
        if (get(verticalOffset, r) != EMPTY) {
            return false;
        }
        if (get(c, horizontalOffset) != EMPTY) {
            return false;
        }
        if (get(verticalOffset, horizontalOffset) != EMPTY) {
            return false;
        }
        return true;
    }
    ...
    void setBlock(char c, char r) {
        ...
        char horizontalOffset = (char) ('1' + '7' - r);
        set(c, r, BLOCKED);
        set(c, horizontalOffset, BLOCKED);
        set(verticalOffset, r, BLOCKED);
        set(verticalOffset, horizontalOffset, BLOCKED);
    ...
    /** Copy of the starting board. */
    private PieceColor[] _copyBoard;
    /** Total number of moves.*/
    private int _numMoves;
    /** Stack of numJump Values. */
